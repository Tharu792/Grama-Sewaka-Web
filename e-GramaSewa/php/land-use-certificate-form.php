...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $purpose = $_POST['purpose']; // Dropdown selection for land work purpose
    ...
    elseif (!$message) {
        $uploadDir = 'uploads/land_use_certificates/';
        if (!is_dir($uploadDir)) {
        ...
        // Handle file uploads for land deed
        if (isset($_FILES['land_deed']) && $_FILES['land_deed']['error'] == 0) {
            $fileName = time() . "_land_deed_" . basename($_FILES['land_deed']['name']);
            $targetFile = $uploadDir . $fileName;
            $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

            if (!in_array($fileType, $validTypes)) {
                $message = "Only PDF, JPG, and PNG files are allowed for land deed.";
            } elseif (!move_uploaded_file($_FILES['land_deed']['tmp_name'], $targetFile)) {
                $message = "Error uploading land deed.";
            } else {
                $uploadedFiles[] = $fileName; // Store land deed
            }
        }

        // Handle file uploads for electricity bill
        ...
        // Handle file uploads for NIC photo
        if (isset($_FILES['nic_photo']) && $_FILES['nic_photo']['error'] == 0) {
            $fileName = time() . "_nic_photo_" . basename($_FILES['nic_photo']['name']);
            $targetFile = $uploadDir . $fileName;
            $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

            if (!in_array($fileType, $validTypes)) {
                $message = "Only JPG, JPEG, and PNG files are allowed for NIC photo.";
            } elseif (!move_uploaded_file($_FILES['nic_photo']['tmp_name'], $targetFile)) {
                $message = "Error uploading NIC photo.";
            } else {
                $uploadedFiles[] = $fileName; // Store NIC photo
            }
        }
        ...
        if ($uploadedFiles && !$message) {
            ...
            $stmt = $pdo->prepare("INSERT INTO land_use_certificates 
                (username, full_name, dob, nic, land_location, purpose, contact, proof_document, district, divisional_secretariat, category, status) 
                VALUES (:username, :full_name, :dob, :nic, :land_location, :purpose, :contact, :proof_document, :district, :divisional_secretariat, 'land_use', 'Pending')");
...
<title>Land Use Certificate Application</title>
<style>
    /* Track App Style for Land Use Certificate Form - No Animations */
    :root {
    ...
    .land-purpose-select:hover {
        border-color: var(--secondary);
        box-shadow: 0 0 10px rgba(16, 185, 129, 0.2);
    }

    .land-purpose-select:focus {
        border-color: var(--primary);
        box-shadow: 0 0 15px rgba(37, 99, 235, 0.3);
        outline: none;
        transform: translateY(-2px);
...
<div class="container">
<h2>Land Use Certificate Application</h2>
...
<div id="guidelinesModal" class="modal" style="display: none;">
    <div class="modal-content" style="background-color:#fff; color:#000; padding:20px; border-radius:8px; width:80%; margin:10% auto; box-shadow:0 5px 15px rgba(0,0,0,0.3); font-family: Arial, sans-serif;">
        ...
        <h3>Land Use Certificate Guidelines</h3>
        ...
        <p><strong>Land Location:</strong> Specify whether it's your own land or other land, and provide detailed location information.</p>
        <p><strong>Purpose for Land Work:</strong> Select the appropriate purpose from the dropdown (Construction, Fencing, or Other).</p>
        <p><strong>Land Deed:</strong> Upload a clear copy of your land deed document.</p>
        <p><strong>Electricity Bill:</strong> Upload a recent electricity bill as proof of residence.</p>
        <p><strong>NIC Photo:</strong> Upload a clear photo of your NIC or Passport.</p>
...
<form action="" method="POST" enctype="multipart/form-data">
    ...
    <label>Land Location (Own Land / Other Land):</label>
    <textarea name="land_location" required></textarea>

    <label>Purpose for Land Work:</label>
    ...
    <label>Upload Land Deed:</label>
    <input type="file" name="land_deed" accept=".jpg,.jpeg,.png,.pdf" required>

    <label>Upload Electricity Bill:</label>
    <input type="file" name="electricity_bill" accept=".jpg,.jpeg,.png,.pdf" required>

    <label>Upload NIC Photo:</label>
    <input type="file" name="nic_photo" accept=".jpg,.jpeg,.png" required>
