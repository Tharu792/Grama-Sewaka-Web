...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $divisional_secretariat = htmlspecialchars($_POST['divisional_secretariat']);
    ...
    elseif (!$message) {
        $uploadDir = 'uploads/income_certificates/';
        if (!is_dir($uploadDir)) {
        ...
        $uploadedFiles = [];
        for ($i = 1; $i <= 3; $i++) {
            if (isset($_FILES['proof_document' . $i]) && $_FILES['proof_document' . $i]['error'] == 0) {
                $fileName = time() . "_" . basename($_FILES['proof_document' . $i]['name']);
                ...
                if (!in_array($fileType, $allowedTypes)) {
                    ...
                    break;
                } elseif (!move_uploaded_file($_FILES['proof_document' . $i]['tmp_name'], $targetFile)) {
                    $message = "Error uploading file: " . $_FILES['proof_document' . $i]['name'];
                    break;
                } else {
        ...
        if ($uploadedFiles && !$message) {
            $proofDocumentsJSON = json_encode($uploadedFiles);

            // Insert form data and uploaded files into the database
            $stmt = $pdo->prepare("INSERT INTO income_certificates 
                (username, full_name, dob, nic, income, contact, purpose, proof_document, district, divisional_secretariat, status) 
                VALUES (:username, :full_name, :dob, :nic, :income, :contact, :purpose, :proof_document, :district, :divisional_secretariat, 'Pending')");
...
<head>
    ...
    <style>
        /* Track App Style for Income Certificate Form - No Animations */
        :root {
...
<div id="guidelinesModal" class="modal" style="display: none;">
    <div class="modal-content" style="background-color:#fff; color:#000; padding:20px; border-radius:8px; width:80%; margin:10% auto; box-shadow:0 5px 15px rgba(0,0,0,0.3); font-family: Arial, sans-serif;">
        ...
        <h3>Income Certificate Guidelines</h3>
        ...
        <p><strong>NIC Number:</strong> Enter your National Identity Card number (minimum 12 characters).</p>
        <p><strong>Income (Monthly):</strong> Enter your monthly income amount in numbers only.</p>
        <p><strong>Contact Number:</strong> Enter a valid 10-digit contact number.</p>
        <p><strong>Purpose of Certificate:</strong> Specify the purpose for which you need this income certificate.</p>
        <p><strong>Proof Documents:</strong> Upload supporting documents like income reports, tax returns, and NIC copy.</p>
...
<form action="" method="POST" enctype="multipart/form-data">
    ...
    <label>NIC Number (min 12 characters):</label>
    ...
    <label>Upload Proof Documents (Income Report):</label>
    <input type="file" name="proof_document1" accept=".pdf,.jpg,.jpeg,.png" required>
    <label>Upload Proof Documents ( NIC ):</label>
    <input type="file" name="proof_document2" accept=".pdf,.jpg,.jpeg,.png">
    <label>Upload Proof Documents (Tax returns):</label>
    <input type="file" name="proof_document3" accept=".pdf,.jpg,.jpeg,.png">

    <small>You can upload multiple documents. Each file will be stored separately.</small>
