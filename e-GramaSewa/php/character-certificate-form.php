...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif (!$message) {
        $uploadDir = 'uploads/character_certificates/';
        if (!is_dir($uploadDir)) {
        ...
        // If files are uploaded successfully, insert data into database
        if ($uploadedFiles && !$message) {
            ...
            $stmt = $pdo->prepare("INSERT INTO character_certificates 
                (username, full_name, dob, nic, permanent_address, current_address, contact, purpose, proof_document, district, divisional_secretariat, status) 
                VALUES (:username, :full_name, :dob, :nic, :permanent_address, :current_address, :contact, :purpose, :proof_document, :district, :divisional_secretariat, 'Pending')");
...
<head>
    ...
    <title>Character Certificate Application</title>
    <style>
        /* Track App Style for Character Certificate Form - No Animations */
        :root {
...
<div id="guidelinesModal" class="modal" style="display: none;">
    <div class="modal-content" style="background-color:#fff; color:#000; padding:20px; border-radius:8px; width:80%; margin:10% auto; box-shadow:0 5px 15px rgba(0,0,0,0.3); font-family: Arial, sans-serif;">
        ...
        <h3>Character Certificate Guidelines</h3>
        ...
        <p><strong>Purpose of Certificate:</strong> Specify the purpose for which you need this character certificate (employment, visa, etc.).</p>
        <p><strong>Proof Documents:</strong> Upload supporting documents including ID, utility bill, and birth certificate.</p>
...
<form action="" method="POST" enctype="multipart/form-data">
    ...
    <label>Upload Proof Documents ( ID ):</label>
    <input type="file" name="proof_document1" accept=".pdf,.jpg,.jpeg,.png" required>
    <label>Upload Proof Documents ( Utility Bill):</label>
    <input type="file" name="proof_document2" accept=".pdf,.jpg,.jpeg,.png">
    <label>Upload Proof Documents ( Birth_certificate):</label>
